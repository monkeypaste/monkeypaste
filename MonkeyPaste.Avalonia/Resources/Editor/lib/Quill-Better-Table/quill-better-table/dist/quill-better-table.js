...
/***/ (function (module, __webpack_exports__, __webpack_require__) {
			...
			class table_column_tool_TableColumnTool {
				...
				updateToolCells() {
					let tableContainer = external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.find(this.table);
					// TAK START
					if (!tableContainer) {
						if (getTableElements().length > 0) {
							tableContainer = external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.find(getTableElements()[0]);
						}
						if (!tableContainer) {
							return;
						}
						if (typeof tableContainer.colGroup !== 'function') {
							return;
						}
					}
					// TAK END
					...
					let cur_cell_node = null;
					for (let index = 0; index < Math.max(cellsNumber, existCells.length); index++) {
						cur_cell_node = cur_cell_node ? cur_cell_node.next : CellsInFirstRow.head;

						let col = tableCols.at(index);
						// TAK START
						let colWidth = 10;
						if (cur_cell_node && cur_cell_node.domNode) {
							colWidth = cur_cell_node.domNode.getBoundingClientRect().width
						} else if (col && col.formats()[col.statics.blotName]) {
				...
				addColCellHolderHandler(cell) {
					...
					const handleMouseup = e => {
						...
						// TAK START
						if (dragging && colBlot && typeof colBlot.format === 'function') {
						// TAK END
...
/* harmony default export */ var header = (header_Header);
			...
			class table_selection_TableSelection {
				...
				mouseDownHandler(e) {
					...
					// TAK START
					if (isDragging()) {
						log('table mouse down rejected, data dragging');
						return;
					}
					if (globals.IsTableDragSelecting == null) {
						// prevent select
						log('table mouse down rejected, selection dragging');
						return
					}
					if (this.selectedTds.find(x => x.domNode == e.target.closest('td[data-row]')) != null) {
						// prevent select2 
						log('table mouse down rejected, drag starting');

						this.quill.root.parentNode.removeEventListener('mousemove', mouseMoveHandler, false);
						this.quill.root.parentNode.removeEventListener('mouseup', mouseUpHandler, false);
						return;
					}
					const self = this;
					this.quill.root.parentNode.addEventListener('mousemove', mouseMoveHandler, false);
					this.quill.root.parentNode.addEventListener('mouseup', mouseUpHandler, false);
					this.downTd = e.target.closest('td[data-row]');
					...
					function mouseMoveHandler(e) {
						...
						const mp = getClientMousePos(e);
						if (!isPointInRect(cleanRect(getTableElements()[0].getBoundingClientRect()), mp)) {
							//debugger;
						}
						//const endTd = e.target.closest('td[data-row]');
						const endTd = getElementClosestToPoint(mp, 'td[data-row]');
						...
						// TAK START
						if (!endTd || !self.startTd) {
							return;
						}
						const start_rect = screenToEditorRect(cleanRect(self.startTd.getBoundingClientRect()));
						const end_rect = screenToEditorRect(cleanRect(endTd.getBoundingClientRect()));
						self.boundary = toBtRect(rectUnion(start_rect, end_rect));

						// TAK END
						self.selectedTds = self.computeSelectedTds();
						self.repositionHelpLines(); // avoid select text in multiple table-cell

						if (self.startTd !== endTd) {
							self.quill.blur();
						}
					}

					function mouseUpHandler(e) {
						self.quill.root.parentNode.removeEventListener('mousemove', mouseMoveHandler, false);
						self.quill.root.parentNode.removeEventListener('mouseup', mouseUpHandler, false);
						self.dragging = false;
						const endTd = getElementClosestToPoint(getClientMousePos(e), 'td[data-row]');
						if (self.downTd == endTd && self.selectedTds && self.selectedTds.length > 1) {
							//self.selectedTds = [endTd];
							const end_rect = screenToEditorRect(cleanRect(endTd.getBoundingClientRect()));
							self.boundary = toBtRect(rectUnion(end_rect, end_rect));
							self.selectedTds = self.computeSelectedTds();
				...
				correctBoundary() {
					...
					if (!tableContainer) {
						...
						tableContainer = getTableElements()[0];
					}

					if (!tableContainer || typeof tableContainer.descendants !== 'function') {
						return;
					}
					// TAK END
				...
				refreshHelpLinesPosition() {
					...
					if (this.selectedTds.length == 0) {
						let td_blot_to_select = quillFindBlot(getTableCellElementAtDocIdx(getDocSelection().index));
						if (!td_blot_to_select) {
							return;
						}
						this.selectedTds.push(td_blot_to_select);
					}
					// TAK END
			...
			class quill_better_table_BetterTable extends Module {
				...
				getOpsMenu(elm) {
					...
					const path = getAncestorElements(elm);
					if (!path || path.length <= 0) return;
					const tableNode = path.filter(node => {
						return node.tagName && node.tagName.toUpperCase() === 'TABLE' && node.classList.contains('quill-better-table');
					})[0];
					...
					if (isNullOrUndefined(this.tableSelection)) {
						this.tableSelection = new table_selection_TableSelection(tableNode, this.quill, this.options);
					}
					...
					if (isNullOrUndefined(this.columnTool)) {
						this.columnTool = new table_column_tool_TableColumnTool(tableNode, this.quill, this.options);
					}
