...
window.superCm = (function (msie) {
	...
	var cmTemplate = $("<div>")
		...
		.append($("<div>").addClass("context-menu-options"));

	var cmOptTemplate = $("<div>");

	var cmSearchTemplate = $("<div>")
		.addClass("context-menu-search")
		.append(
			$("<input>").prop({
				type: "text",
	...
	function setCurrentActiveOver(cmIndex, optIndex) {
		if (
			activeOpt == null ||
			activeOpt.cmIndex != cmIndex ||
			activeOpt.optIndex != optIndex
		) {
			if (activeOpt != null) {
	...
	function updateCm(cmIndex) {
		...
		opts.forEach(function (opt, optIndex) {
			...
			if (separator) {
				...
				cmOptElement.append(optSeparatorTemplate.clone());
			...
			if (icon) {
				...
				if (opt.iconBgColor !== undefined) {
					iconElm.css("background-color", opt.iconBgColor);
					iconElm.css("border-radius", "3px");
					iconElm.css("border", "1px solid black");
				}

				if (opt.iconFgColor !== undefined) {
				...
				cmOptElement.on("submenu", function () {
					...
					showCm(opt.submenu, submenuIndex, {
						...
						y:
							...
							parseInt(getOptContainer(cmIndex).css("padding-top"))
					});
				});
	...
	function updateCmPosition(cmIndex, repositionX, repositionY) {
		...
		if (repositionX) {
			...
			if (
				cm.position.x - $(window).scrollLeft() + cmElementWidth >=
				$(window).innerWidth()
			) {
		...
		if (repositionY) {
			if (
				cm.position.y - $(window).scrollTop() + cmElementHeight >=
				$(window).innerHeight()
			) {
				...
				if (cmIndex > 0) {
					var paddingBottom = parseInt(
						getOptContainer(cmIndex).css("padding-bottom")
					);
					var lastOpt = getOptElements(cmIndex).last();
					var paddingTop = parseInt(
						getOptContainer(cmIndex).css("padding-top")
					);
	...
	function isSelectable(cmIndex, optIndex) {
		...
		return (
			opt.separator === undefined &&
			(opt.disabled === undefined || !opt.disabled)
		);
	}
	...
	function activeUp() {
		...
		var previousOptIndex = findSuitableSelectable(
			activeOpt.cmIndex,
			activeOpt.optIndex - 1,
			true
		);
	...
	function activeDown() {
		...
		var nextOptIndex = findSuitableSelectable(
			activeOpt.cmIndex,
			activeOpt.optIndex + 1,
			false
		);
	...
	$(document).on(
		"mousedown.scm contextmenu.scm",
		".context-menu, .opt-text, .opt-icon, .opt-separator",
		function (e) {
			e.stopPropagation();
		}
	);
	...
	$(document).on("keydown.scm", function (e) {
		...
		if (cms.length > 0) {
			if (e.key == "ArrowUp" || e.which == 38) {
				...
				e.preventDefault();
				getOptElement(activeOpt.cmIndex, activeOpt.optIndex).click();
	...
	///rv:11/.test(navigator.userAgent)
	false
	// TAK END
);
