...
namespace MonkeyPaste.Avalonia {
    public static class MpAvMouseHook_Win32 {
        ...
        private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam) {
            ...
            if (nCode >= 0 && MouseMessage.WM_LBUTTONDOWN == (MouseMessage)wParam) {
                ...
                RightMouseClicks++;
            } else if (nCode >= 0 && MouseMessage.WM_MOUSEWHEEL == (MouseMessage)wParam) {
                // mouseData = 4287102976 +120
                // mouseData = 7864320 -120
                MpConsole.WriteLine("MouseData: " + hookStruct.mouseData);
                if(hookStruct.mouseData == 4287102976) {
                    OnGlobalMouseWheelScroll?.Invoke(typeof(MpAvMouseHook_Win32).ToString(), 120.0d);
                } else if (hookStruct.mouseData == 7864320) {
                    OnGlobalMouseWheelScroll?.Invoke(typeof(MpAvMouseHook_Win32).ToString(), -120.0d);
                } else {
                    Debugger.Break();
                }

            } else if (nCode >= 0 && MouseMessage.WM_MOUSEMOVE == (MouseMessage)wParam) {
                GlobalMouseLocation.X = (double)hookStruct.pt.x;
                GlobalMouseLocation.Y = (double)hookStruct.pt.y;

                OnGlobalMouseMove?.Invoke(typeof(MpAvMouseHook_Win32).ToString(), GlobalMouseLocation);
            }
