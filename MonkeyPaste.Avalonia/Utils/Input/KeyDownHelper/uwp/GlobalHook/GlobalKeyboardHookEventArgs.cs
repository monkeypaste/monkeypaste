...
namespace MonkeyPaste.Avalonia {
    ...
    class GlobalKeyboardHookEventArgs : HandledEventArgs {
        ...
        public Key AvaloniaKey { get; private set; }
        ...
        public GlobalKeyboardHookEventArgs(
            GlobalKeyboardHook.LowLevelKeyboardInputEvent keyboardData,
            GlobalKeyboardHook.KeyboardState keyboardState,
            Key avkey) : this(keyboardData, keyboardState) {
            AvaloniaKey = avkey;
        }
    }
    class GlobalKeyboardHook : IDisposable {
        ...
        // NOTE setting hook as static to prevent GC
        // see https://stackoverflow.com/a/29413516/105028
        ...
        public IntPtr LowLevelKeyboardProc(int nCode, IntPtr wParam, IntPtr lParam) {
            ...
            if (Enum.IsDefined(typeof(KeyboardState), wparamTyped)) {
                object lparam_obj = Marshal.PtrToStructure(lParam, typeof(LowLevelKeyboardInputEvent));
                LowLevelKeyboardInputEvent llkie = (LowLevelKeyboardInputEvent)lparam_obj;

                Key avkey = KeyInterop.KeyFromVirtualKey(llkie.VirtualCode, llkie.Flags);
                Key test = KeyInterop.KeyFromVirtualKey(ToInt32(wParam), ToInt32(lParam));
                Key test2 = KeyInterop.KeyFromVirtualKey(llkie.VirtualCode, ToInt32(lParam));

                var eventArguments = new GlobalKeyboardHookEventArgs(llkie, (KeyboardState)wparamTyped, avkey);
                ...
                var key = (Keys)llkie.VirtualCode;
                if (RegisteredKeys == null || RegisteredKeys.Contains(key)) {
                    EventHandler<GlobalKeyboardHookEventArgs> handler = KeyboardEvent;
        ...
        private static int ToInt32(IntPtr ptr) {
            // FROM avalonia WindowImpl.AppWndProc.cs
            if (IntPtr.Size == 4)
                return ptr.ToInt32();
