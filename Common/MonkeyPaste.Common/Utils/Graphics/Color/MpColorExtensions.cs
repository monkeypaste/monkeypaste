...
namespace MonkeyPaste.Common {
    public static class MpColorExtensions {
        ...
        public static string ToContrastForegoundColor(this string hex, bool flip = false, bool remove_alpha = false) {
            bool is_bright = hex.IsHexStringBright();
            if (flip) {
                is_bright = !is_bright;
            }
            return is_bright ?
                MpCommonTools.Services.PlatformResource.GetResource<string>(MpThemeResourceKey.ThemeBlackColor.ToString()).ToPortableColor().ToHex(remove_alpha) :
                MpCommonTools.Services.PlatformResource.GetResource<string>(MpThemeResourceKey.ThemeWhiteColor.ToString()).ToPortableColor().ToHex(remove_alpha);
        }
        ...
        public static string ToComplementHexColor(this string hex) {
            ...
            Complement Changes each component of a color to a new paramValue 
            based on the sum of the highest and lowest RGB values in the 
            selected color. Illustrator adds the lowest and highest RGB 
            values of the current color, and then subtracts the paramValue of 
            each component from that number to create new RGB values. 
            For example, suppose you select a color with an RGB 
            paramValue of 102 for red, 153 for green, and 51 for blue. 
            Illustrator adds the high (153) and low (51) values, to 
            end up with a new paramValue (204). Each of the RGB values in the 
            existing color is subtracted from the new paramValue to create new 
            complementary RGB values: 204 – 102 (the current red paramValue) = 102 
            for the new red paramValue, 204 – 153 (the current green paramValue) = 51 
            for the new green paramValue, and 204 – 51 (the current blue paramValue) = 153 
            for the new blue paramValue.
            */
