...
namespace MonkeyPaste.Common.Wpf {
    public static class MpWpfClipoardImageHelper {
        public static object GetWpfBitmap(byte[] bytes) {
            return bytes.ToBitmapSource();
        }
        public static System.Drawing.Bitmap GetSysDrawingBitmap(byte[] bytes) {
        ...
        public static DataObject GetClipboardImage_WinForms(Bitmap image, Bitmap imageNoTr, DataObject data) {
            //Clipboard.Clear();
            if (data == null) {
                data = new DataObject();
            }
            ...
            MemoryStream dibMemStream = new MemoryStream();
        ...
        private static Bitmap CF_DIBV5ToBitmap(byte[] data) {
            // from https://stackoverflow.com/a/14335591/105028
            GCHandle handle = GCHandle.Alloc(data, GCHandleType.Pinned);
            var bmi = (WinApi.BITMAPV5HEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(WinApi.BITMAPV5HEADER));
