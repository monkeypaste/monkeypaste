...
namespace MonkeyPaste.Common.Avalonia {
    public static partial class MpAvPlatformDataObjectExtensions {
        ...
        public static async Task WriteToPasteboardAsync(this Dictionary<string, object> ido, bool isDnd) {
            await Task.Delay(1);
            ...
            foreach (string format in formats) {
                switch (format) {
                    ...
                    case MpPortableDataFormats.Image: {
                            ...
                            ido.AddOrReplace(format, imgBase64.ToBytesFromBase64String());
                            break;
                        }
                    default: {
                            if (!ido.TryGetValue(format, out string dataStr)) {
                                continue;
                            }
                            ...
                            //ido.Set(format, arr);
                            ido.AddOrReplace(format, dataStr);
                            break;
                        }
