...
namespace MonkeyBuild {
    ...
    internal class MonkeyBuild {

        #region Properties

        static List<MpBuildFlags> BuildTasks { get; set; } = [];
        ...
        static bool DO_REMOTE_PACKAGING => BuildTasks.Contains(MpBuildFlags.DO_REMOTE_PACKAGING);
        static bool FORCE_REPLACE_REMOTE_TAG => BuildTasks.Contains(MpBuildFlags.FORCE_REPLACE_REMOTE_TAG);

        static bool DO_LOCAL_VERSIONS => BuildTasks.Contains(MpBuildFlags.DO_LOCAL_VERSIONS);
        static bool DO_REMOTE_VERSIONS => BuildTasks.Contains(MpBuildFlags.DO_REMOTE_VERSIONS);
        ...
        static void ProcessAll() {
            ...
            if (BuildTasks.Contains(MpBuildFlags.GEN_ADDON_LISTING)) {
                GenAddOnListings();
            }
            if (BuildTasks.Contains(MpBuildFlags.GEN_PROD_LISTING)) {
                GenProdListing();
            }
            if (BuildTasks.Contains(MpBuildFlags.TRANSLATE_RESX)) {
                TranslateAllResxWrapper();
            }
            if (BuildTasks.Contains(MpBuildFlags.GEN_EMPTY_RESX)) {
                GenAllEmptyLocalizedResx();
            }
            if (BuildTasks.Contains(MpBuildFlags.GEN_LOCALIZED_MANIFESTS)) {
                LocalizeManifests();
            }
            if (BuildTasks.Contains(MpBuildFlags.VERIFY_CONSISTENT_CULTURES)) {
                VerifyConsitentCultures();
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_LOCAL_PACKAGING)) {
                PublishLocal();
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_LOCAL_LEDGER)) {
                GenInvLedger(false);
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_REMOTE_PACKAGING)) {
                PublishRemote();
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_REMOTE_LEDGER)) {
                GenInvLedger(true);
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_LOCAL_VERSIONS)) {
                UpdateVersions(false);
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_REMOTE_VERSIONS)) {
                UpdateVersions(true);
            }
            if (BuildTasks.Contains(MpBuildFlags.DO_LOCAL_CULTURE_INDEX)) {
...
{0}
        ...
        static string PushReleaseToRemote(string plugin_name, string forced_new_version = null) {
            ...
            if (exit_code == 1) {
                // version exist
                if (BuildTasks.Contains(MpBuildFlags.FORCE_REPLACE_REMOTE_TAG)) {
