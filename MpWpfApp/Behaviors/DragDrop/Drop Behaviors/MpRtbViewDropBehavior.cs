...
namespace MpWpfApp {
    public class MpRtbViewDropBehavior : MpDropBehaviorBase<MpRtbView> {
        ...
        public async Task MergeContentItem(MpCopyItem mci, bool isDuplicating) {
            ...
            // merge content
            if (isHomeMerge) {
                AssociatedObject.BindingContext.CopyItem.ItemData = MpHelpers.Instance.CombineRichText(AssociatedObject.Rtb.Document.ToRichText(), mci.ItemData);
            } else {
                AssociatedObject.BindingContext.CopyItem.ItemData = MpHelpers.Instance.CombineRichText(mci.ItemData, AssociatedObject.Rtb.Document.ToRichText());
            }
