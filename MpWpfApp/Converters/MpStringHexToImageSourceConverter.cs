    ...
    public class MpStringHexToImageSourceConverter : IValueConverter {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture) {
            if (value is string hexStr && hexStr.IsStringHexColor()) {
                ...
                var bgBmp = (BitmapSource)new BitmapImage(new Uri(MpPrefViewModel.Instance.AbsoluteResourcesPath + @"/Images/texture.png"));
                bgBmp = bgBmp.Tint(((SolidColorBrush)brush).Color, false);
                var borderBmp = (BitmapSource)new BitmapImage(new Uri(MpPrefViewModel.Instance.AbsoluteResourcesPath + @"/Images/textureborder.png"));
                ...
                if (parameter is string paramStr) {
                    ...
                    if (!string.IsNullOrEmpty(checkPath)) {
                        var checkBmp = (BitmapSource)new BitmapImage(new Uri(MpPrefViewModel.Instance.AbsoluteResourcesPath + checkPath));
                        if (!MpWpfColorHelpers.IsBright((brush as SolidColorBrush).Color)) {
