...
#pragma warning disable CS0618 // Obsoleted InputModifiers
			...
			//KeyModifiers state = e.Modifiers;
...
#pragma warning restore CS0618
		...
		public static CefDragData GetCefDragData(this DragEventArgs e)
		{
			...
			if(TryGetFileDropData(e.Data, out IEnumerable<string> fileNames))
			{
				foreach(string filePath in fileNames)
				{
					...
					// TAK START
					dragData.AddFile(filePath.Replace("\\", "/"), Uri.EscapeDataString(filePath));
					// TAK END
				}
			}
		...
		private static bool TryGetFileDropData(IDataObject data, out IEnumerable<string> fileNames)
		{
			...
			else if(data.Contains(CefNetDragData.DataFormatFiles))
			{
				object file_obj = data.Get(CefNetDragData.DataFormatFiles);
				if(file_obj is IEnumerable<string> fnl)
				{
					fileNames = fnl;
				}
				else if(file_obj is IEnumerable<IStorageItem> sil)
				{
					fileNames = sil.Select(x => x.TryGetLocalPath());
				}
				else
				{
					throw new NotImplementedException($"unknown files format '{file_obj.GetType()}'");
				}
			}
			else
			{
		...
		private static string GetUrlString(IDataObject data, string[] formats)
		{
			...
			if(formats.Contains(CefNetDragData.DataFormatUrl) &&
				data.Get(CefNetDragData.DataFormatUrl) is byte[])
			{
