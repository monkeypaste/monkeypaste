...
namespace AvCoreAnnotator {
    ...
    public class AvCoreAnnotatorPlugin : MpIAnalyzeComponent {
        public MpAnalyzerPluginResponseFormat Analyze(MpAnalyzerPluginRequestFormat req) {
            ...
            string content_ref_url = req.GetRequestParamStringValue((int)AvAnnnotaterParameterType.RefUrl);
            if(string.IsNullOrWhiteSpace(content_ref_url)) {
                throw new Exception("Cannot annotate w/o ref to content");
            }
            ...
            if (resp.annotations != null && 
                ...
                // mark that the data is for request item
                resp.dataObject.SetData(MpPortableDataFormats.LinuxUriList, new List<string> { content_ref_url });
                return resp;
            }
