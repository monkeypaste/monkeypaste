...
namespace iosKeyboardTest {
    public class MenuViewModel : ViewModelBase, IKeyboardViewRenderer, IKeyboardRenderSource {
        ...
        public IEnumerable<string> CompletionDisplayValues {
            get {
                string leading_word = GetLeadingWord(LastTextInfo,false);
        ...
        #region State
        TextRange LastTextInfo { get; set; }
        bool IsScrolling =>
        ...
        public void ShowCompletion(TextRange textInfo) {
            ...
            results = HandleAutoCorrect(textInfo, results, autoCorrectResult);            
        ...
        bool WasSpace(TextRange curRange,TextRange lastRange) {
        ...
        IEnumerable<string> HandleAutoCorrect(TextRange textInfo, IEnumerable<string> results, string autoCorrectResult) {
        ...
        void StartAutoCorrect(TextRange curRange, string autoCorrectedText) {
            // get incorrect text
            ...
            string out_val = GetCompletionDisplayValue(text_to_delete, autoCorrectedText, false);
        ...
        void FinishAutoCorrect(TextRange curRange, bool is_undo) {
            if(is_undo) {
        ...
        void DoAutoCorrect(TextRange textInfo) {
            DoCompletion(textInfo, LastAutoCorrectRange.SelectedText.Trim());
        }
        void DoCompletion(TextRange textInfo, string completionText) {
            if(completionText == ConfirmIgnoreAutoCorrectCompletionText) {
                // cancel auto correct by inserting a space
                InputConnection.OnText(" ");
                return;
            }
            if(GetLeadingWord(textInfo) is { } orig_leading_text) {
