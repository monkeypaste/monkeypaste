...
namespace MonkeyPaste {    
    public class MpClient : MpDBObject {
        ...
        public MpClient(int clientId) {
            ...
            DataTable dt = MpLogFormController.Db.Execute("select * from MpClient where MpClientId=" + clientId);
        ...
        public override void WriteToDatabase() {
            if(Ip4Address == null || Ip4Address == string.Empty || MpLogFormController.Db.NoDb) {
                Console.WriteLine("MpTag Error, cannot create nameless tag");
                return;
            }
            if(ClientId == 0) {
                MpLogFormController.Db.ExecuteNonQuery("insert into MpClient(fk_MpPlatformId,Ip4Address,AccessToken,LoginDateTime) values(" + PlatformId + ",'" + Ip4Address + "','" + AccessToken + "','"+ this.LoginDateTime.ToString("yyyy-MM-dd HH:mm:ss")+"')");
                ClientId = MpLogFormController.Db.GetLastRowId("MpClient","pk_MpClientId");
            }
            else {
                LogoutDateTime = DateTime.Now;
                MpLogFormController.Db.ExecuteNonQuery("update MpClient set LogoutDateTime='" + LogoutDateTime.Value.ToString("yyyy-MM-dd HH:mm:ss") + "'");
            }
        }
