    ...
    public class MpSetting:MpDBObject {
        ...
        public override void WriteToDatabase() {
            ...
            if(this.SettingId == 0) {
                ...
                DataTable dt = MpLogFormController.Db.Execute("select * from MpSetting where SettingName='" + this.SettingName + "'");
                if(dt.Rows.Count > 0) {
                    ...
                    MpLogFormController.Db.ExecuteNonQuery("insert into MpSetting(SettingName,SettingValue) values ('" + this.SettingName + "','" + this.SettingValue + "')");
                    this.SettingId = MpLogFormController.Db.GetLastRowId("MpSetting","pk_MpSettingId");
                    isNew = false;
                }
            }
            else {
                MpLogFormController.Db.ExecuteNonQuery("update MpSetting set SettingName='" + this.SettingName + "',SettingValue='" + this.SettingValue + "' where pk_MpSettingId=" + this.SettingId);
            }
            if(isNew) {
                MapDataToColumns();
            }
