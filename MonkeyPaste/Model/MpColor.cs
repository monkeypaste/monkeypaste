...
namespace MonkeyPaste {
    public class MpColor:MpDBObject {
        ...
        public MpColor(int colorId) {
            DataTable dt = MpLogFormController.Db.Execute("select * from MpColor where pk_MpColorId=" + colorId);
        ...
        public override void WriteToDatabase() {
            if(ColorId == 0) {
                DataTable dt = MpLogFormController.Db.Execute("select * from MpColor where R=" + _r + " and G=" + _g + " and B=" + _b + " and A=" + _a);
                if(dt != null && dt.Rows.Count > 0) {
                    ColorId = Convert.ToInt32(dt.Rows[0]["pk_MpColorId"].ToString());
                }
                else {
                    MpLogFormController.Db.ExecuteNonQuery("insert into MpColor(R,G,B,A) values(" + _r + "," + _g + "," + _b + "," + _a + ")");
                    ColorId = MpLogFormController.Db.GetLastRowId("MpColor","pk_MpColorId");
                }
            }
            else {
                MpLogFormController.Db.ExecuteNonQuery("update MpColor set R=" + _r + ", G=" + _g + ", B=" + _b + ", A=" + _a + " where pk_MpColorId=" + ColorId);
            }
        }
