...
namespace MonkeyPaste {
    public class MpFileListItemRowPanelController : MpController {
        public MpFileListItemRowPanel FileListItemRowPanel { get; set; }

        public MpFileListItemRowIcon IconPictureBox { get; set; }

        public MpFileListItemRowLabel PathTextBox { get; set; }

        private int _rowId = -1;

        public MpFileListItemRowPanelController(int tileId,int rowId, string path,MpController Parent) : base(Parent) {
            _rowId = rowId;

            FileListItemRowPanel = new MpFileListItemRowPanel(tileId) {
                //FlowDirection = FlowDirection.LeftToRight,
                BorderStyle = BorderStyle.FixedSingle,
                Anchor = AnchorStyles.Top                
            };

            IconPictureBox = new MpFileListItemRowIcon(tileId) {
            ...
            PathTextBox = new MpFileListItemRowLabel(tileId) {
