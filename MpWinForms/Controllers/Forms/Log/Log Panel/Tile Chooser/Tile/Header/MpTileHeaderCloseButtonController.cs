...
namespace MonkeyPaste {
    public class MpTileHeaderCloseButtonController : MpController {
        public MpButton CloseButton { get; set; }
        public delegate void ButtonClicked(object sender,EventArgs e);
        public event ButtonClicked ButtonClickedEvent;

        public MpTileHeaderCloseButtonController(MpController parentController) : base(parentController) {
            CloseButton = new MpButton() {
                Margin = new Padding(10),
                Padding = Padding.Empty,
                TabIndex = 1,
                BackColor = Color.Transparent,//MpHelperSingleton.Instance.IsBright(((MpTilePanelController)((MpTileHeaderPanelController)Parent).Parent).CopyItem.ItemColor.Color) ? Color.Black:Color.White,
            ...
            CloseButton.DoubleBuffered(true);
