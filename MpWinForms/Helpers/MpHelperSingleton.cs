...
namespace MonkeyPaste {
    ...
    public class MpHelperSingleton {
        ...
        //and delta position
        public Rectangle ProjectRectangle(Rectangle r,int dw,int dh) {
        ...
        public Keys GetKeyFromChar(char character,string useCulture = "en-US") {
            short keyNumber = WinApi.VkKeyScanEx(character, WinApi.LoadKeyboardLayout(CultureInfo.GetCultureInfo(useCulture).KeyboardLayoutId.ToString("X8"),1));
            if (keyNumber == -1) {
                return System.Windows.Forms.Keys.None;
            }
            return (System.Windows.Forms.Keys)(((keyNumber & 0xFF00) << 8) | (keyNumber & 0xFF));
        }  
        public string GetTextStringFromKey(Keys key) {
        ...
        public List<ModifierKeys> GetModListFromKey(Keys key) {
            List<ModifierKeys> modList = new List<ModifierKeys>();
            if ((key & Keys.Shift) != Keys.None) {
                modList.Add(ModifierKeys.Shift);
            }
            if ((key & Keys.Alt) != Keys.None) {
                modList.Add(ModifierKeys.Alt);
            }
            if ((key & Keys.Control) != Keys.None) {
                modList.Add(ModifierKeys.Control);
            }
            if ((key & Keys.LWin) != Keys.None) {
                modList.Add(ModifierKeys.Win);
            } else if ((key & Keys.RWin) != Keys.None) {
                modList.Add(ModifierKeys.Win);
            }
            return modList;
        }
