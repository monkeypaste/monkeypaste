...
"MSWin32-x64-multi-thread-10.0.17134.112" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.17134.112) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEADDRNOTAVAIL WSAELOOP WSAEFAULT WSAECONNABORTED WSAENOTSOCK
	WSAETOOMANYREFS WSAEMSGSIZE WSAEPROVIDERFAILEDINIT WSAETIMEDOUT
	WSAECONNREFUSED WSAEINVALIDPROVIDER WSAENOTEMPTY WSAEINTR WSAESTALE
	WSAENAMETOOLONG WSAENETUNREACH WSAEUSERS WSAESOCKTNOSUPPORT
	WSAENOTCONN WSAEDQUOT WSAENOBUFS WSAEDISCON WSAEREMOTE WSAEISCONN
	WSAEPROTOTYPE WSAECONNRESET WSAEINVALIDPROCTABLE WSAEDESTADDRREQ
	WSAESHUTDOWN WSAEHOSTUNREACH WSAEOPNOTSUPP WSAEREFUSED WSAEINVAL
	WSAEBADF WSAENETDOWN WSAEWOULDBLOCK WSAENOMORE WSAEADDRINUSE
	WSAEINPROGRESS WSAEALREADY WSAENETRESET WSAEPROCLIM WSAEMFILE
	WSAEPFNOSUPPORT WSAECANCELLED WSAEPROTONOSUPPORT WSAEACCES
	WSAENOPROTOOPT WSAEAFNOSUPPORT WSAEHOSTDOWN
    )],
);
